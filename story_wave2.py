...
def transition_to_wave2(screen):
    clock = pygame.time.Clock()
    fade_surface = pygame.Surface((800, 600))
    fade_surface.fill((0, 0, 0))
    red_flash = pygame.Surface((800, 600))
    red_flash.fill((255, 0, 0))

    for strength in [50, 100, 150, 100, 50]:
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                pygame.quit()
                sys.exit()
        red_flash.set_alpha(strength)
    ...
    for alpha in range(255, -1, -5):
        ...
        pygame.display.flip()
        clock.tick(60)
    return start_wave_2_directly(screen)

def start_wave_2_directly(screen):
        global punch_count
        ...
        class Fighter:
            def __init__(self, sprites, x, y, foot_offset, char_name, is_player=True):
                ...
                self.health = 100 if is_player else 300
                self.flying = False
                self.fly_timer = 0
                self.fly_cooldown = 300
            ...
            def delayed_damage(self, opponent):
                if hasattr(self, "delayed_punch_frame") and self.delayed_punch_frame > 0:
                    self.delayed_punch_frame -= 1
                    if self.delayed_punch_frame == 0:
            ...
            def update(self, opponent):
                ...
                if not self.is_player:
                    if not self.flying and random.random() < 0.002 and self.fly_cooldown <= 0:
                        self.flying = True
                        self.fly_timer = 600
                    if self.flying:
                        self.fly_timer -= 1
                        ...
                        if self.fly_timer <= 0:
                            self.flying = False
                            self.fly_cooldown = 600
                    else:
                        if self.fly_cooldown > 0:
                            self.fly_cooldown -= 1

                if not self.is_player:
                    self.delayed_damage(opponent)
            ...
            def draw_top_health(self, screen):
                if self.char_name != "demon_archon":
                    return
                bar_width = 200
                bar_height = 20
        ...
        archon_sprites = load_sprites("demon_archon")

        ground_y = 500
        nyborg = Fighter(nyborg_sprites, 100, ground_y, foot_offsets["nyborg"], "nyborg", is_player=True)
        archon = Fighter(archon_sprites, 600, ground_y, foot_offsets["demon_archon"], "demon_archon", is_player=False)
        archon.health = 300
        ...
        while running:
            ...
            if not game_over:
                if keys[pygame.K_SPACE] and keys[pygame.K_a] and keys[pygame.K_s] and nyborg.dodge_penalty == 0:
                    ...
                    if keys[pygame.K_a] and nyborg.dodge_penalty == 0:
                        ...
                        nyborg.state = "idle"
                    if keys[pygame.K_z] and not bullet_active and player_shot_cooldown == 0:
                        bullet_active = True
                        bullet_rect = pygame.Rect(nyborg.rect.centerx, nyborg.rect.centery, 10, 4)

                        dx = archon.rect.centerx - nyborg.rect.centerx
                        dy = archon.rect.centery - nyborg.rect.centery
                        distance = max(1, (dx ** 2 + dy ** 2) ** 0.5)
                        bullet_velocity = [dx / distance * 12, dy / distance * 12]
                        player_shot_cooldown = 30

                    if player_shot_cooldown > 0:
                    ...
                    if bullet_rect.colliderect(archon.rect) and archon.flying:
                            archon.take_damage(0.5)
                            bullet_active = False
                    elif bullet_rect.x > 800 or bullet_rect.x < 0 or bullet_rect.y > 600 or bullet_rect.y < 0:
                            bullet_active = False
            ...
            if game_over:
                ...
                text = game_over_font.render("Thanks for playing!", True, (255, 215, 0))
                text_rect = text.get_rect(center=(screen.get_width() // 2, screen.get_height() // 2 - 60))
                screen.blit(text, text_rect)

                score_font = pygame.font.SysFont("Arial", 24)
                punch_text = score_font.render(f"Total Punches: {punch_count}", True, (255, 255, 255))
                punch_rect = punch_text.get_rect(center=(screen.get_width() // 2, screen.get_height() // 2))
                screen.blit(punch_text, punch_rect)

                quit_text = score_font.render("Press Q to Quit", True, (200, 200, 200))
